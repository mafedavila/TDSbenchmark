...
def train_model(data, dataset_path, experiment):
    ...
    # Extract columns from the experiment configuration
    categorical_columns = experiment['columns'].get('categorical', [])
    log_columns = experiment['columns'].get('log', [])
    mixed_columns = experiment['columns'].get('mixed', {})
    general_columns = experiment['columns'].get('general', [])
    non_categorical_columns = experiment['columns'].get('non_categorical', [])
    integer_columns = experiment['columns'].get('integer', [])
    problem_type = {experiment['problem_type'].capitalize(): experiment['target']}
